...
plt.scatter(nm_ld['b_vol'],nm_ld['cyto_vol'],color='r',marker='.',label = 'LVD')
plt.scatter(nm_hd['b_vol'],nm_hd['cyto_vol'],color='b',marker='.', label = 'HVD')

slope, intercept, r_value, p_value, std_err = stats.linregress(np.log(nm_ld['b_vol']),np.log(nm_ld['cyto_vol']))
slopec, interceptc, r_valuec, p_valuec, std_errc = stats.linregress(np.log(nm_hd['b_vol']),np.log(nm_hd['cyto_vol']))
...
plt.ylabel('Cytoplasm volume ($\mu m^3$)')
...
plt.text(0.03,0.13, yText1, fontsize=8,color='b')
plt.text(0.03,0.23, yText2, fontsize=8,color='r')
#plt.savefig('../../figures/v2/figures/new/final/final_final/rel_mito_vs_rel_ves.png',dpi =600)
plt.ylim(0.0001, 15)

plt.legend(loc = 'lower right')
plt.savefig('./Figures/fig_4_2.png', dpi =600)
#plt.savefig('./Figures/fig_4_2.png', dpi =600)
plt.show()
