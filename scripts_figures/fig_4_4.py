...
data['extra_cyto'] = data['cyto_vol'] - 0.002*data['convex_hull_inter_mito']

nm_ld = data.loc[(data['Mito'] == 'No')  & (data['final_med_mean_dis'] > 0.033) & (data['extra_cyto'] > 1e-4)]
...
plt.plot(nm_ld['b_vol'],nm_ld['extra_cyto'],'r.', label = 'LVD')
plt.plot(nm_hd['b_vol'],nm_hd['extra_cyto'],'b.', label = 'HVD')

slope, intercept, r_value, p_value, std_err = stats.linregress(np.log(nm_ld['b_vol']),np.log(nm_ld['extra_cyto']))
slopec, interceptc, r_valuec, p_valuec, std_errc = stats.linregress(np.log(nm_hd['b_vol']),np.log(nm_hd['extra_cyto']))
...
plt.ylabel('Extra cytoplasmic volume ($\mu m^3$)')
...
plt.text(0.03,0.15, yText1, fontsize=8,color='b')
plt.text(0.03,0.3, yText2, fontsize=8,color='r')
#plt.savefig('../../figures/v2/figures/new/final/final_final/rel_mito_vs_rel_ves.png',dpi =600)
plt.ylim(0.0001, 15)
plt.legend(loc='lower right')
plt.savefig('./Figures/fig_4_4.png', dpi =600)
plt.show()
