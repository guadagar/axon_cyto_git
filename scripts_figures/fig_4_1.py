...
plt.scatter(nm_ld['b_vol'],nm_ld['convex_hull_inter_mito'],color='r',marker='.', label = 'LVD')
plt.scatter(nm_hd['b_vol'],nm_hd['convex_hull_inter_mito'],color = 'b',marker='.', label = 'HVD')

slope, intercept, r_value, p_value, std_err = stats.linregress(np.log(nm_ld['b_vol']),np.log(nm_ld['convex_hull_inter_mito']))
slopec, interceptc, r_valuec, p_valuec, std_errc = stats.linregress(np.log(nm_hd['b_vol']),np.log(nm_hd['convex_hull_inter_mito']))
...
plt.ylabel(r'Convex cloud volume ($\mu m^3$)')
plt.xlabel(r'Total bouton volume ($\mu m^3$)')
...
plt.text(0.03,0.3, yText1, fontsize=8,color='b')
plt.text(0.03,0.5, yText2, fontsize=8,color='r')
plt.xscale('log')
plt.yscale('log')
plt.legend(loc = 'lower right')
