...
plt.plot(m_ld['b_vol'],m_ld['cyto_vol_mito'],'r.', label = 'LVD')
plt.plot(m_hd['b_vol'],m_hd['cyto_vol_mito'],'b.', label = 'HVD')

slope, intercept, r_value, p_value, std_err = stats.linregress(np.log(m_ld['b_vol']),np.log(m_ld['cyto_vol_mito']))
slopec, interceptc, r_valuec, p_valuec, std_errc = stats.linregress(np.log(m_hd['b_vol']),np.log(m_hd['cyto_vol_mito']))
...
plt.text(0.07,0.3, yText1, fontsize=8,color='b')
plt.text(0.07,0.4, yText2, fontsize=8,color='r')
#plt.savefig('../../figures/v2/figures/new/final/final_final/rel_mito_vs_rel_ves.png',dpi =600)

plt.ylim(0,0.6)
#plt.ylim(0,0.17)
plt.legend(loc='lower right')
plt.savefig('./Figures/fig_5_2.png', dpi =600)
plt.show()