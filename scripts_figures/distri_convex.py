...
sns.histplot(data = ltp['convex_hull_inter_mito'],stat='probability', kde=True,bins= bins, color ='red')
sns.histplot(data = c['convex_hull_inter_mito'],stat='probability', kde=True,bins=bins, color
='blue')
#plt.plot(0.033*np.ones(len(np.arange(0,0.2,1e-3))), np.arange(0,0.2,1e-3), color = 'k', ls ='-.')
#ax.set_yticks(np.arange(-20,121,20))
#axs.set_xticks(np.arange(0.0, 0.1, 0.005))
#plt.xlim(0.015,0.1)
#plt.ylim(0,0.25)
plt.ylabel('Frequency')
plt.xlabel(r'Ves Cloud Vol ($\mu m^3$)')
plt.savefig('./Figures/distri_convex.png',dpi =600)
plt.show()
